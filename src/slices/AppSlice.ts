...
`;
    ...
    // const circSupply = parseFloat(graphData.data.protocolMetrics[0]?.WALTCirculatingSupply);
    ...
    //   provider,
    // );
    const sohmMainContract = new ethers.Contract(addresses[networkID].SWALT_ADDRESS as string, sOHMv2, provider);
    const ohmMainContract = new ethers.Contract(addresses[networkID].WALT_ADDRESS as string, sOHMv2, provider);
    ...
    // const sohmOldContract = new ethers.Contract(addresses[networkID].OLD_SWALT_ADDRESS as string, sOHM, provider);

    const ohmBalance = await ohmMainContract.balanceOf(addresses[networkID].STAKING_ADDRESS);
